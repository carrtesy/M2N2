...
python train.py dataset=yahoo_20 window_size=5 stride=1 eval_stride=1 batch_size=8 epochs=30 log_freq=1 normalization=None model=MLP model.latent_dim=2 &&
python train.py dataset=yahoo_20 window_size=5 stride=1 eval_stride=1 batch_size=8 epochs=30 log_freq=1 normalization=Detrend model=MLP model.latent_dim=2